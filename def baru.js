				...
				if (data.length>0) {
					for (var i = 0; i < data.length; i++) {
						...
						if (product_list.length>0) {
							...
							hasil += '<tr><th>Nama Item</th><th>Ket.</th><th>Qty</th><th>Harga</th></tr>'
							grand_price = 0;
							$.each(product_list,function(key,val){
								var price = (val.price*val.qty);
									...
									grand_price += price;
								hasil += '<tr detail-item-in-cart-'+val.id+'><td><a href="'+val.link_url+'">'+val.product_name+'</a></td><td align="center">'+val.option_name+'</td><td align="right">'+val.qty+'</td><td align="right">'+convert_Rp(Math.ceil(price))+'</td></tr>';
							})
							hasil += '<tr><td align="right" colspan="3">Total</td><td align="right">'+convert_Rp(Math.ceil(grand_price))+'</td></tr></table></div><a href="'+base_url+'keranjang?inv='+data[i].trans_cd+'" class="btn btn-outline-success btn-sm">Check Out</a>';
...
$(document).on('click','.p-option',function(event){
	event.preventDefault();
	$('.product-price , [product-stock-display], .product-price-before-discount').css({'opacity':'0','-webkit-animation':"die_time .3s"});
	var price = $(this).attr('price'),
		price_before_discount = $(this).attr('price-before-discount'),
		stock = $(this).attr('stock-option');
	setTimeout(function(){
		$('.product-price , [product-stock-display], .product-price-before-discount').css({'opacity':'1','-webkit-animation':"show_time .5s"});
...
function render_single_cart_item(val,trans_status){
	...
	return '<tr single-item-on-cart-list-'+val.id+'><td style="width:80px"><div class="img-col" style="border:1px solid #ddd"><img src="'+val.small_file_url+'"></div></td><td class="text-uppercase"><a href="'+val.link_url+'">'+val.product_name+' '+val.option_name+'</a><span class="pull-right">'+btn+'</span></td><td style="width:6em">'+qty+'</td><td align="right" render-single-price-item-on-cart-'+val.id+'>'+convert_Rp(val.total_price)+'</td></tr>';
}
function render_single_cart_item_tipe_2(val,trans_status){
	var qty = val.qty+val.unit_cd;
	return '<tr single-item-on-cart-list-'+val.id+'><td class="text-uppercase"><a href="'+val.link_url+'">'+val.product_name+' '+val.option_name+'</a></td><td>'+qty+'</td><td align="right" render-single-price-item-on-cart-'+val.id+'>'+convert_Rp(val.total_price)+'</td></tr>';
}
...
function after_submit(form_name,data){
	...
	if (form_name=='delete_from_cart') {
		...
		if (data=='1') {
			...
			confirm_result("Tidak bisa menyimpan terkirim",3,3000);			
		}
